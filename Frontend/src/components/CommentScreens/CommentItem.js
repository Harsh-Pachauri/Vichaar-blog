...
const CommentItem = ({ comment, activeUser }) => {
    ...
    useEffect(() => {
        ...
        const getCommentLikeStatus = async () => {
            ...
            try {
                const { data } = await axios.post(`https://vichaar-blog.onrender.com/comment/${comment_id}/getCommentLikeStatus`, { activeUser }, {
                    headers: {
    ...
    const handleCommentLike = async () => {
        ...
        try {
            const { data } = await axios.post(`https://vichaar-blog.onrender.com/comment/${comment_id}/like`, { activeUser }, {
                headers: {
                    "Content-Type": "application/json",
                    authorization: `Bearer ${localStorage.getItem("authToken")}`,
                },
                        ...
                        [...Array(5)].map((_, index) => {
                            return (
                                <FaStar
                                    ...
                                    className="star"
                                    size={15}
                                    color={comment.star > index ? "#cd670e" : "grey"}
                                />
                            )
                        })
                    }
